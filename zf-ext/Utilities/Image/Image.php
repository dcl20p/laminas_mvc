 ...
 *    @github https://github.com/sunhater/kcfinder/blob/master/lib/class_image.php
 */
...
abstract class Image
{
    ...
    public function __construct(mixed $image, array $options = [])
    {
    ...
    {
        $driver = ucfirst($driver); // Different
        $class = __NAMESPACE__ . "\\Image{$driver}"; // Different
    ...
    final protected function buildImage($image)
    {
        ...
        if ($image instanceof $class) {
            ...
            list($key, $width) = [key($image), current($image)]; // Different
            list($key, $height) = [key($image), current($image)]; // Different
    ...
    static function getInstanceFilename($filename, $dir = null, $tpl = null)
    {
        if ($tpl === null)
            $tpl = "{name}_{sufix}_{ext}";
        $fullPath = ($dir === null);
        if ($fullPath)
